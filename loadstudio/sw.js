const CACHE='loadstudio-complete-v252';
...
self.addEventListener('fetch',e=>{
  ...
  // Network-first for navigation so a cold open never returns undefined from
  // an empty cache, which causes "Safari can't open the page." Cache-first is
  // only safe once the cache is populated; before that it returns undefined
  // → respondWith(undefined) → Safari error. Network-first with cache fallback
  // is the reliable pattern (mirrors ACR Reader sw.js).
