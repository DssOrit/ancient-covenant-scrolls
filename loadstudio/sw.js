...
// Browsers reject a redirected response from a SW for navigation requests
// ("response served by service worker has redirected"). Re-wrap the body as
// a plain Response so the redirected flag is cleared before we hand it back.
function clean(res){
...
self.addEventListener('fetch',e=>{
  ...
  // Network-first for navigation: avoids "Safari can't open the page" on cold
  // open (empty cache → respondWith(undefined) → error). clean() strips the
  // redirected flag that fires "response served by service worker has redirected".
  if(req.mode==='navigate'){
    e.respondWith(fetch(req).then(res=>clean(res).then(c=>{
      if(c&&c.status===200&&c.type==='basic'){const clone=c.clone();caches.open(CACHE).then(cache=>cache.put(req,clone)).catch(()=>{});}
      return c;
