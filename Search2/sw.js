...
self.addEventListener('activate', function(e) {
  e.waitUntil(
    // Scoped cleanup: only ever touch this app's own cache prefix, never
    // another app's cache sharing the same origin.
    caches.keys().then(function(keys) {
      return Promise.all(keys.filter(function(k) { return k.indexOf('acr-search2-') === 0 && k !== CACHE; }).map(function(k) { return caches.delete(k); }));
